...
const canViewAdmin = computed(() => userRoles.value.includes('admin') || userRoles.value.includes('manager'));
    ...
    >
      ...
      <div class="flex items-center justify-between p-4 border-b border-gray-800">
        ...
        <Button
          ...
          @click="toggleSidebar"
        >
          <component :is="isExpanded ? ChevronRight : ChevronDown" class="h-5 w-5" />
        </Button>
      </div>

      <!-- Navigation -->
      <nav class="p-4 space-y-2 overflow-y-auto h-[calc(100vh-8rem)]">
        <div v-for="item in menuItems" :key="item.title" v-show="item.show" class="space-y-1">
          <Collapsible :default-open="true">
            ...
            >
              ...
              <span v-show="isExpanded || isHovered" class="truncate flex-1 text-left">{{ item.title }}</span>
            </CollapsibleTrigger>
            <CollapsibleContent>
      ...
      <div class="absolute bottom-0 left-0 right-0 p-4 border-t border-gray-800 bg-gray-900">
        <div v-show="isExpanded || isHovered" class="text-xs text-gray-400 truncate mb-2">
          {{ user?.email }}
        </div>
        <Button
          variant="ghost"
